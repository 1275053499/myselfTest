...
//  铺皇精简版
//
//  Created by selice on 2017/12/30.
...
@interface YJLScrollmg()<UIScrollViewDelegate>
//轮播的图片数组
...
@implementation YJLScrollmg

#pragma mark - 初始化方法
//创建用来缓存图片的文件夹
+ (void)initialize {
    cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"YJLcacheimg"];
    BOOL isDir = NO;
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            if (_placeholderImage) {
                ...
                UIImage *placeholderImage = [UIImage imageNamed:@"Xunxian_背影.jpeg"];
...
- (void)imageClick {
    ...
    } else if ([self.yjlscrollDelegate respondsToSelector:@selector(loopView:clickImageAtIndex:)]){
        
        [self.yjlscrollDelegate loopView:self clickImageAtIndex:self.currIndex];
    }
}
